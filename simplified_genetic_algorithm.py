...
from db_operations import fetch_data_from_db, insert_timetable_into_db, get_course_classes, DB_PATH
...
def fitness(schedule):
    ...
    for teacher, days in teacher_slots.items():
        for day, slots in days.items():
            ...
            for i in range(1, len(slots)):
                ...
                if consecutive_count > 3:
                    # Apply a much stronger soft penalty for each additional consecutive class
    ...
    for teacher, day_counts in teacher_day_counts.items():
        if len(day_counts) > 1:  # Only check if teacher teaches on multiple days
            ...
            # Square root to get standard deviation, multiply by 12.0 to make this a significant soft constraint
    ...
    for branch, day_data in branch_slots.items():
        for day, slots in day_data.items():
            ...
            # Ideally, classes should be evenly distributed, not clumped together
            slots.sort()
    ...
    # Bonus for teacher diversity (teaching multiple courses)
...
def validate_schedule(schedule, fitness_score):
    ...
    if find_room_conflicts(schedule):  # <- Using your function
        return False
